...
from ...responses import Error, Errors, HttpResponse, ValidateError
...
class AwsApiGatewayHttpRequest(HttpRequest):
    ...
    def parse_request(cls, request):
        ...
        qs_params = (
            ...
            if request["multiValueQueryStringParameters"]
            else {}
        )
...
class AwsApiGwHttpResponse(HttpResponse, CamelCasedModel):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.headers is not None:
            _dict["multiValueHeaders"] = {
                k: list(v) for k, v in _dict.pop("headers").items()
            }
...
class HttpCorrelationIdMiddleware(BaseMiddleware):
    ...
    def on_request(self):
        self.handler.correlation_id = self.handler.raw_request["multiValueHeaders"].get(
            self.header, None
        )
...
class LambdaApiGwProxyHandler(BaseFunctionHandler, abc.ABC):
    ...
    def __serialize_request__(self, request, context):
        if not issubclass(self.request_class, AwsApiGatewayHttpRequest):
            raise Exception(
                f"request must be subclass of {AwsApiGatewayHttpRequest.__name__}"
            )
        ...
        if self.path_params_class is not None:
            try:
                self.request.params = self.path_params_class.parse_obj(
                    self.request.params
                )
        ...
        if self.query_string_class is not None:
            try:
                self.request.query_str = self.query_string_class.parse_obj(
                    self.request.query_str
                )
    ...
    def __process_handle__(self):
        try:
            ...
            match = re.match(
                r"handle\(\) takes \d+ positional arguments? but \d+ were given", str(e)
            )
            if match:
                raise Exception(
    ...
    def handle(
        self, *args, **kwargs
    ) -> Union[AwsApiGwHttpResponse, Tuple[int, Any], Any]:
        pass

    def handle_error(
        self, error: Union[Exception, BaseFunctionError, ValidationError]
    ) -> AwsApiGwHttpResponse:
        ...
        try:
            ...
            response.body = Errors(
                errors=[Error(type=type(error).__name__, message=str(error))]
            )
            ...
            response.body = Errors(
                errors=[
                    ValidateError(loc=i["loc"], type=i["type"], message=i["msg"])
                    for i in e.errors()
                ]
            )
            ...
            response.body = Errors(
                errors=[Error(type=type(e).__name__, message="Internal Server Error")]
            )
            response.status_code = 500

        return response

    def __process_response__(
        self, response: Union[AwsApiGwHttpResponse, Tuple[int, Any], Any]
    ) -> AwsApiGwHttpResponse:
