...
from faas_framework.aws.handlers.api_gw import (
...
class ProxyHandler(LambdaApiGwProxyHandler):
    ...
    def handle(
        self, body, path_params, query_string, **kwargs
    ) -> Union[AwsApiGwHttpResponse, Tuple[int, Any], Any]:
        return 200, body
...
@pytest.mark.parametrize(
    "event,expected_output,handler",
    [
        (
            pytest.lazy_fixture("api_gw_event_dict_str_body"),
            "Hello from Lambda!",
            MyLambdaProxyHandler(),
        ),
        (
            pytest.lazy_fixture("api_gw_event_dict_json_body"),
            body_payload,
            ProxyHandler(),
        ),
        (
            pytest.lazy_fixture("api_gw_event_dict_json_base64_body"),
            body_payload,
            ProxyHandler(),
        ),
    ],
...
@pytest.mark.parametrize(
    "handler_class,expected_response",
    [
        (NoHttpResponseHandler, AwsApiGwHttpResponse(body={"some_key": "some_value"})),
        (
            TupleResponse,
            AwsApiGwHttpResponse(body={"some_key": "some_value"}, status_code=300),
        ),
        (
            HttpResponseHandler,
            AwsApiGwHttpResponse(body={"some_key": "some_value"}, status_code=202),
        ),
        (
            HttpDefaultResponseHandler,
            AwsApiGwHttpResponse(body={"some_key": "some_value"}, status_code=200),
        ),
    ],
)
def test_responses(
    handler_class, expected_response, api_gw_event_dict_json_body, lambda_context
):
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            AwsApiGwHttpResponse(
                body={
                    "timestamp": "2021-02-21T11:40:38.568+0000",
                    "errors": [
                        {
                            "type": "BaseFunctionError",
